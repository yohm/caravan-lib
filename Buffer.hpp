...
#ifndef CARAVAN_LIB_BUFFER_HPP
#define CARAVAN_LIB_BUFFER_HPP
...
namespace caravan_impl {
  class Buffer {
    public:
    Buffer(int _parent, Logger &_logger) : parent(_parent), logger(_logger) {};
    const int parent;
    Logger &logger;

    std::queue<task_t> tasks;
    std::queue<TaskResult> task_results;

    void EnqueueInitialTasks(int n_consumers) {
    ...
    void Run(const std::vector<int> &consumers) {
      ...
      while (true) {
        logger.d("%d tasks, %d running_tasks, %d requesting_workers, %d task_results, %d consumers, request_sent %d",
                 tasks.size(),
        ...
        if (received) {
          ...
          if (has_tasks_to_send) {  // Sending a task to a Consumer
            ...
            TaskResult tr = task_results.front();
            SendResult(tr, &send_req, send_buf);
            task_results.pop();
          } else {  // must not happen
            assert(false);
            MPI_Abort(MPI_COMM_WORLD, 1);
    ...
    long SendTask(const int worker, MPI_Request *p_req, std::vector<uint8_t> &send_buf) {
      ...
      logger.d("task %d is assigned to %d", t.first, worker);
      return t.first;
    }
    ...
    void SendResult(const TaskResult &res, MPI_Request *p_req, std::vector<uint8_t> &send_buf) {
      ...
      MPI_Isend(send_buf.data(), send_buf.size(), MPI_BYTE, parent, MsgTag::BUF_PROD_SEND_RESULT, MPI_COMM_WORLD,
                p_req);
